...
const ChatOverlay = ({ onClose }: { onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-white flex flex-col animate-slide-up">
       ...
       <div className="p-3 bg-white border-t flex gap-2"><input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&send()} className="flex-1 bg-gray-100 rounded-full px-4 py-2 outline-none" placeholder="輸入訊息..." /><button onClick={send} className="w-10 h-10 bg-italian-red text-white rounded-full flex items-center justify-center shadow"><Icon name="chevronLeft" className="w-5 h-5 rotate-180" /></button></div>
    </div>
  );
};
